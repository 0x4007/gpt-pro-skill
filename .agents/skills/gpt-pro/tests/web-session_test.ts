...
function fixture() {
  return {
    ...
    cookie: "oai-did=fixture-device; __Secure-oai-is=ois1.fixture.AAAAAAAAAAAAAAAA.signature; session=fixture; scoped=first; scoped=second",
    headers: {
    ...
    for (const account of [undefined, "fixture-account"]) {
      ...
      globalThis.fetch = (input, init) => {
        ...
        if (
          ...
          request.headers.get("authorization") !== `Bearer ${value.accessToken}` ||
...
Deno.test("invalid web snapshots fail without exposing credential content", () => {
  const inputs = ["", "CHATGPT_WEB_SESSION='SECRET_INVALID_JSON'", encode(fixture()) + "\n" + encode(fixture())];
  for (const mutate of [
